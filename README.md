![GitHub repo size](https://img.shields.io/github/repo-size/Shimu-I/PythonMastery)
![GitHub last commit](https://img.shields.io/github/last-commit/shimu-i/PythonMastery)
![Visitor Count](https://visitor-badge.laobi.icu/badge?page_id=shimu-i/PythonMastery)


# My Python Documentation

Learning through multiple YouTube resources to master Python from **basics to advanced**, including automation, APIs, data science, and analytics.

---

## 1. Python Full Tutorials

**Channel:** [Telusko](https://youtube.com/playlist?list=PLsyeobzWxl7poL9JTVyndKe62ieoN-MZ3&si=DPE9iJKXBtSbUZTp)

*(YouTube Playlist – Python for Beginners)*

---

## 2. Python Course (Now Watching)

**Channel:** [Code with Harry](https://youtu.be/UrsmFxEIp5k?si=u0ZVx3YvlCfmU4BE)

*(Complete Python Course in One Video)*

---

## 3. Python API Development

**Course:** [Python API Development – Comprehensive Course for Beginners](https://youtu.be/0sOvCWFmrtA?si=EOO1OJM9KL3JG4Vq)

*(Covers FastAPI, CRUD operations, and backend fundamentals)*

---

## 4. Python for Data Science

**Course:** [Learn Python for Data Science – Full Course for Beginners](https://youtu.be/CMEWVn1uZpQ?si=XPPRgpMl3Uba9UnU)

*(Data analysis, visualization, and machine learning foundations)*

---

## 5. Data Analyst Bootcamp

**Course:** [Data Analyst Bootcamp for Beginners](https://youtu.be/PSNXoAs2FtQ?si=rHwK-HValLIFy4Bf)

*(SQL, Tableau, Power BI, Python, Excel, Pandas, Projects & more)*

---

## 6. Python Automation

**Course 1:** [Python Automation Tutorial – How to Automate Tasks for Beginners (Full Course)](https://youtu.be/s8XjEuplx_U?si=_q87u89RtjUmqYOa)

**Course 2:** [Automate with Python – Full Course for Beginners](https://youtu.be/PXMJ6FS7llk?si=t0ste2W-6eGZxqbs)

*(Learn to automate repetitive tasks, file handling, web automation, and more)*

---

## 7. Python Deep Learning (TensorFlow & Keras)

**Course 1:** [TensorFlow 2.0 Complete Course – Python Neural Networks for Beginners Tutorial](https://youtu.be/tPYj3fFJGjk?si=jeLJamxTiuM3u1RX)

**Course 2:** [Python TensorFlow for Machine Learning – Neural Network Text Classification Tutorial](https://youtu.be/VtRLrQ3Ev-U?si=aSDnVbYwmQjKZnB4)

**Course 3:** [Keras with TensorFlow Course – Python Deep Learning and Neural Networks for Beginners Tutorial](https://youtu.be/qFJeN9V1ZsI?si=FACbsR-sX77J7HjD)

*(Covers neural networks, TensorFlow 2.0, and Keras for building deep learning models)*




## Study Record

| Day no and date | Topic Covered | Time spent | YouTube Timestamp | Exercises Done | Satisfaction Level |
| --- | --- | --- | --- | --- | --- |
| Automate the boring stuff with python by Al Sweigart --- Early Access Edition, 11/08/2024 |
| Day-1 -- 8 dec, 2025 | chapter - 1 (Python Basics) | 1h 30min | N/A | book exercises | good |
| Day-2 -- 12 dec, 2025 | chapter - 1 (Python Basics) | 2h 20min | N/A | c-1-practice question 1 to 10 | good |
| Day-3 -- 14 dec, 2025 | chapter - 2 (if-else and Flow Control) | 1h | N/A | book exercises | good |


✅ *Continuing to document my Python learning journey step by step.*
Google doc link: https://docs.google.com/document/d/1J771T7q3s3AdgrNXZkIs5WB4DnarU6VmJAXr13uEbkA/edit?usp=sharing
